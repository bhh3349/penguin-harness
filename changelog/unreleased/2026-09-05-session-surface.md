...
A surface Session carries no model reference (`sessions.surface` is a new column, `NULL` for a conversation) — how it reaches databases that already exist is recorded in [the compatibility entry](2026-09-05-backward-compatibility.md) and no core Session: the SessionManager never drives it, and every run-shaped call (Task, compaction, steer, approval) answers 409 `surface_session`. Its state is the surface's own, pushed into the SessionManager so `statusOf` — and every list row — answers for it, and published as the same `session_state` event a run's flip is, so the sidebar's running and unread glyphs are unchanged. `POST /api/sessions/:id/surface` opens it (idempotent, an optional first prompt), `GET` reports it, `DELETE` closes it; deleting the Session closes it too.

## The chat page consumes contributions

The Web App now fetches `GET /api/contributions` once per signed-in user. Its `pages` are folded into the router — a page a pushed platform or a plugin contributes mounts as long as this build carries its renderer — and its `sessionSurfaces` become rows in a picker on the New chat page, a third pill beside the Agent and the Workspace, so a surface is opened in a Workspace the user chose. The pill appears only when a plugin contributes a surface. A surface Session's chat route renders the surface's renderer (`TerminalSurface` attaches the terminal view the dock uses; an `iframe` renderer loads the plugin's own page) instead of the message stream and composer. A Session created on a MACHINE opens its surface there, since the surface routes follow the Session, and the pty is recorded as that machine's so its byte stream is relayed rather than looked for on this server.

A terminal can now run a program instead of a shell: `Terminals.create` takes a `command` (argv, no login shell), `env` and `unsetEnv`, used by a surface, never exposed over HTTP. A pty no longer inherits the server's `TMUX` / `TMUX_PANE` either — it is not a pane of whatever multiplexer started the server, and a program that believes otherwise emits passthrough sequences for something that is not there.

## The claude-code plugin

`@prismshadow/penguin-plugin-claude-code` (`plugins/claude-code`) contributes the `claude-code` surface: a "New chat" entry that runs `claude` (or `PENGUIN_CLAUDE_BIN`) in the Session's Workspace, with the first prompt as its first argument and as the Session title. Its state is a heuristic — output within a window is running, silence is idle, exit is idle — since a pty cannot report thinking. It scrubs the parent's Claude Code session markers (`CLAUDECODE`, `CLAUDE_CODE_SESSION_ID`, the messaging socket and token, and the rest) from the pty: a harness started from inside a Claude Code session would otherwise hand them to the child, which reads them as being nested and turns transcript saving off. Configuration a deployment sets on purpose, such as `CLAUDE_CODE_USE_BEDROCK`, is inherited untouched. It ships with every build and is listed in the built-in plugin index, but is **not installed by default**: an operator installs it like any other plugin.
