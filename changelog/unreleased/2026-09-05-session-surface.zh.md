...
Web App 现在每个登录用户取一次 `GET /api/contributions`。其 `pages` 并入路由——推送上来的平台或插件贡献的页面，只要本构建有它的渲染器就能挂载——其 `sessionSurfaces` 成为「新建对话」页上一个选择器的选项，与 Agent、Workspace 并列的第三颗药丸，因此表面总是在用户选定的 Workspace 里打开。只有当插件确实贡献了表面时，这颗药丸才出现。表面 Session 的会话路由渲染该表面的渲染器（`TerminalSurface` 挂上停靠栏同款终端视图；`iframe` 渲染器加载插件自己的页面），而不是消息流与输入框。建在某台机器上的 Session 会在那台机器上打开它的表面——表面路由跟随 Session——并把 pty 记为那台机器的，于是字节流走中继，而不是在本服务端上白找。
