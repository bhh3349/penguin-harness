...
- 菜单里原有的两个动作——**安装 penguin 命令…** 和 **检查桌面版更新…**——在桌面版自己的窗口里通过命令面板（Ctrl+P）提供。**在 GitHub 上查看项目** 在任何环境的面板里都有。
- 面板通过服务端到达 shell，走的是更新中继已有的那条通道：`GET /api/desktop/shell` 说明 shell 能做什么，`POST /api/desktop/shell/install-cli` 请它安装命令。两者和更新路由一样只对 shell 自己的窗口开放。
- 从应用里打开的预览窗口同样隐藏菜单栏。
